...
#print(candidates_list)
    ...
    # Name output file
output_path = os.path.join("output", "pypoll_election.csv")
output_file = "election_"

write_pypoll_election = f"{output_file}pypoll_results.txt"

    # Open write file
filewriter = open(write_pypoll_election, mode = 'w')

    # Print to write file
filewriter.write("Election Results\n")
filewriter.write("--------------------------\n")
filewriter.write(f"Total Votes: {voter_count}\n")
for i in range(len(candidates_list)):
    for j in range(len(candidates_list)):
       if (sorted_PERC[i] == counter_candidate[j]):
          sorted_index.append(j)
          filewriter.write(f"{candidates_list[j]} {perc_candidate[j]}% ({counter_candidate[j]})\n")
        #filewriter.write(f"{candidates[count]}: {percentages[count]}% ({vote_counts[count]})\n")
filewriter.write("---------------------------\n")
filewriter.write(f"Winner: {candidates_list[sorted_index[0]]}\n")
filewriter.write("---------------------------\n")
